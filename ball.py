...
class Ball:
    ...
    def __init__(self, canvas, color, paddle, bloak, tkkk):
        """ 初期化処理
        メイン側から Canvas を受け取る。
        ボールの色も str 型で受け取る。
        衝突判定に使うパドルを受け取る。
    ...
    def draw(self):
        ...
        if pos[0] <= block_pos[2] and pos[1] <= block_pos[3] and pos[2] >= block_pos[0] and pos[3] >= block_pos[1]:
    ...
    def failed(self):
        # 動きを止める
        self.canvas.create_text(250, 200, text="GAMEOVER",font=("system",60), tag="go")
        self.x = 0
        self.y = 0
        self.speed = 0
